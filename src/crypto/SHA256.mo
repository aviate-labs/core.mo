...
module {
    ...
    public class SHA256() = {
        private var h : [var Nat32] = Array.thaw<Nat32>(H256);
        ...
        public func reset() : () {
            h := Array.thaw<Nat32>(H256);
            x := Array.init<Nat8>(64, 0);
            nx := 0;
            len := 0;
        };

        public func size() : Nat { 32 };
        ...
        public func checkSum() : [Nat8] {
            ...
            write(BigEndian.fromNat64(n << 3));

            assert (nx == 0);

            let digest = Buffer.Buffer<Nat8>(32);
            label l for (i in h.keys()) {
                let n = BigEndian.fromNat32(h[i]);
                for (v in n.vals()) {
                    digest.add(v);
                };
            };
        ...
        public func write(bs : [Nat8]) : () {
            ...
            if (64 <= (s - start : Nat)) {
                let n = Nat64.toNat(Nat64.fromNat(s - start) & ^ 63);
                block(bs, start, n);
                start += n;
            };
            if (0 < (s - start : Nat)) {
        ...
        private func block(bs : [Nat8], n : Nat, l : Nat) {
            ...
            while (64 <= len) {
                for (i in Iter.range(0, 15)) {
                    let j = i * 4;
                    w[i] := nat8to32(bs[start + j + 0]) << 24 | nat8to32(bs[start + j + 1]) << 16 | nat8to32(bs[start + j +2]) << 8 | nat8to32(bs[start + j +3]);
                };
